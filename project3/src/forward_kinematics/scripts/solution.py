    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        for i in range(len(joints)):
            ...
            try:
                ...
                q = joint_values.position[index]                       #joint values has current posrions so we need to use joint
            except ValueError:                                             #values array
                q = 0.0

            #print q

            if joints[i].type == 'revolute':
                joint_axis = joints[i].axis
                R = tf.transformations.quaternion_matrix(tf.transformations.quaternion_about_axis(q, joint_axis))
            else:
                R = tf.transformations.identity_matrix()

            new_T = tf.transformations.concatenate_matrices(
                tf.transformations.translation_matrix(joints[i].origin.xyz),
                R
            )

            T = tf.transformations.concatenate_matrices(T,new_T)

            all_transforms.transforms.append(convert_to_message(T,child,'world_link'))
